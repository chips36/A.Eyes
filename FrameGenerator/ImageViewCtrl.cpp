...
CRect CImageViewCtrl::GetFittingRect(CRect rect, CSize s_size)
{
	CRect d_rect;
...
void CImageViewCtrl::DisplayImage(cv::Mat s_image, BOOL bDrawNow)
{
	...
	if (!s_image.size().width || !s_image.size().height)
		...
		CRect rect;
		GetClientRect(&rect);
		CRect d_rect = GetFittingRect(rect, CSize(s_image.size().width, s_image.size().height));
...
void CImageViewCtrl::DrawImg(CDC* pDC)
{
	CRect rect;
	GetClientRect(&rect);
	if (m_nBitmapWidth && m_nBitmapHeight) {
		CDC s_dc, d_dc;
		CBitmap d_bitmap;
		CRect d_rect = GetFittingRect(rect, CSize(m_nBitmapWidth, m_nBitmapHeight));
...
void CImageViewCtrl::DrawRectangle(CDC* pDC, int x, int y, int width, int height, COLORREF pen_color, int pen_width, int pen_style)
{
...
void CImageViewCtrl::DrawOSDText(CDC* pDC)
{
	...
	CFont font, * old_font;
	CreateFont("Tahoma", 14, FALSE, font);
